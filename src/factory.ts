...
export function createMCPServer(config: MCPWithLLMConfig = {}) {
  ...
  if (config.llm) {
    try {
      ...
      const _originalProcessUserQuery = server.processUserQuery.bind(server);
      server.processUserQuery = async (query: string, _context?: unknown) => {
        return await llmIntegration.processUserQuery(query, _context);
      };
